    ...
    if(response_code == 200 || response_code == 400) {
    	new Float:isVPN = floatstr(data);
	 	...
	 	if(isVPN < 0) {
			...
			switch(tmp) {
			    case -1: {
			        format(vpnMessage, sizeof(vpnMessage), "{F74222}HATA (Yazý giriþi yapmadýnýz.)");
			    }
			    case -2: {
			        format(vpnMessage, sizeof(vpnMessage), "{F74222}HATA (Hatalý IP adresi)");
			    }
			    case -3: {
			        format(vpnMessage, sizeof(vpnMessage), "{F74222}HATA (Tanýmlanmayan adres / local adres.)");
			    }
			    case -4: {
			        format(vpnMessage, sizeof(vpnMessage), "{F74222}HATA (Databaseye ulaþýlamýyor.)");
			    }
			    case -5: {
			        format(vpnMessage, sizeof(vpnMessage), "{F74222}HATA (IP banlý.)");
			    }
			    case -6: {
			        format(vpnMessage, sizeof(vpnMessage), "{F74222}HATA (Hatalý kullanýcý bilgisi.)");
			    }
				default: {
...
public HttpIPInfo(playerid, response_code, data[])
{
    if(response_code == 200) {
		...
		if(strfind(output[0], "sucess")) {
		    format(dPlayerInfo[targetID[playerid]][Status], 64, "{00FF00}Baþarýlý");
		}
		else {
		    format(dPlayerInfo[targetID[playerid]][Status], 64, "{F74222}Baþarýsýz");
		}
        ...
        new string[144];
  		...
  		format(string, sizeof(string), "[HATA] {FFFFFF}Bu IP üzerinden bilgi alýnamýyor.. (Code: %d) (%s)", response_code, data);
...
COMMAND:dox(playerid, params[])
{
	...
	if(!IsPlayerConnected(targetid))
		return SendClientMessage(playerid, 0xFF0000AA, "Kullanýcý oyunda deðil.");
	...
	format(string, sizeof(string), "Kiþi hakkýnda bilgi alýnýyor %s [%s]", pName, playerIP);
