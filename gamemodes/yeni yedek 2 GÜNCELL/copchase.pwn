...
CMD:tagfix(playerid, params[])
{
  ...
  if(!IsValidDynamic3DTextLabel(Oyuncu[userid][YaraliTag]))
	{
 	DestroyDynamic3DTextLabel(Oyuncu[userid][YaraliTag]);
	AdminMessage(RENK_YARALI, "AdmCmd: %s adlý yönetici %s kullanýcýsýnýn 'yaralý tagý' problemini düzeltti.", Oyuncuadi(playerid), Oyuncuadi(userid));
  }
	if(!IsValidDynamic3DTextLabel(Oyuncu[userid][YardimEdiyor]))
	{
 	DestroyDynamic3DTextLabel(Oyuncu[userid][YardimEdiyor]);
	AdminMessage(RENK_YARALI, "AdmCmd: %s adlý yönetici %s kullanýcýsýnýn 'yardým tagý' problemini düzeltti.", Oyuncuadi(playerid), Oyuncuadi(userid));
  }
	return 1;
}
