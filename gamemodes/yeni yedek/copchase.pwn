...
new Male[TOTAL_M_SKIN] = { // erkek ve kadýn skinleri ekle daha sonra!!
    ...
    297, 298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 311, 21010,
...
public Baslat()
{
	if(OyuncuSayisi() > 2 && OyunBasladi == false && OyunSayac == false)
	{
		...
		new tur = random(10);// Oyuna harita eklerken burayý yükselt.
...
public OnPlayerModelSelectionEx(playerid, response, extraid, modelid)
{
   ...
   {
   if(GetPVarInt(playerid, "aracdegistir") == 1)
      {
     	...
     	switch (reason)
      	{
            ...
            case 2: sebep = "atýldý/yasaklandý";
            default: sebep = "Bilinmiyor";
       	}
		format(eylem, sizeof(eylem), ""#GRI2"%s sunucudan ayrýldý. (%s)", Oyuncuadi(playerid), sebep);
		ProxDetector(15.0, playerid, eylem);
   	}
	OyuncuGuncelle(playerid);
	return 1;
}
...
public OnPlayerSpawn(playerid)
{
	...
	if(Oyuncu[playerid][DM] == true)
	{
		...
		printf("%d, VW:%d",GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid));
	}
	if(Oyuncu[playerid][brutallyWounded] == 1)
	{
...
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid, bodypart)
{
    ...
    GetPlayerArmour(damagedid, armour);
		...
		if(Oyuncu[damagedid][Taser] == true && Oyuncu[damagedid][Polis] == true && Oyuncu[playerid][Polis] == false && Oyuncu[playerid][Taserlendi] == false && SuspectAtes == false && weaponid == 23)
		{
			if(OyuncuYakinMesafe(damagedid, playerid) >= 13.0)
			{
				YollaHataMesaj(damagedid, "Hedefe yeteri kadar yakýn deðilsin.");
				return 0;
			}
			new eylem[130];
			format(eylem, sizeof(eylem), "* Þok tabancasý ile vuruldu. (( %s ))", Oyuncuadi(playerid));
			ProxDetector(10.0, playerid, eylem, EMOTE_RENK);
			TogglePlayerControllable(playerid, 0);
			Oyuncu[playerid][Taserlendi] = true;
			Oyuncu[damagedid][TaserMermiDegis] = true;
			SetTimerEx("TaserVer", TIMER_SANIYE(3), false, "d", damagedid);
			ApplyAnimation(playerid, "CRACK", "crckdeth2", 4.1, 0, 1, 1, 1, 1, 1);
			SetTimerEx("FreezeCoz", TIMER_SANIYE(10), false, "d", playerid);
			SetPlayerHealth(playerid, health);
			return 1;
		}

	if(Oyuncu[damagedid][Beanbag] == true && Oyuncu[damagedid][Polis] == true && Oyuncu[playerid][Polis] == false && Oyuncu[playerid][Beanbaglendi] == false && SuspectAtes == false && weaponid == 25)
	{
		if(OyuncuYakinMesafe(damagedid, playerid) >= 20.0)
		{
			YollaHataMesaj(damagedid, "Hedefe yeteri kadar yakýn deðilsin.");
			return 0;
		}
		new eylem[130];
		format(eylem, sizeof(eylem), "* Plastik mermiler ile vuruldu. (( %s ))", Oyuncuadi(playerid));
		ProxDetector(12.0, playerid, eylem, EMOTE_RENK);
		TogglePlayerControllable(playerid, 0);
		Oyuncu[playerid][Beanbaglendi] = true;
		ApplyAnimation(playerid, "CRACK", "crckdeth2", 4.1, 0, 1, 1, 1, 1, 1);
		SetTimerEx("FreezeCoz", TIMER_SANIYE(10), false, "d", playerid);
		SetPlayerHealth(playerid, health);
		return 1;
	...
	if(EventModu == false && EventModu2 == false && Oyuncu[playerid][Oyunda] == true && Oyuncu[playerid][Polis] == true && Oyuncu[damagedid][Suspect] == true && SuspectAtes == false && weaponid != 0)
	{
		SuspectAtes = Oyuncu[damagedid][SuspectSari] = true;
		new mesaj[150], bolge[30];
		GetPlayer3DZone(playerid, bolge, sizeof(bolge));
		format(mesaj, sizeof(mesaj), "[BODY-ALARM] %s adlý memura ateþ açýldý, tüm birimler yönelsin! [Bölge: %s]", Oyuncuadi(damagedid), bolge);
		PolisTelsiz(mesaj);
		SetPlayerColor(damagedid, SARI);
		SetTimerEx("SuspectSakla", TIMER_SANIYE(15), false, "d", damagedid);
		SetPlayerHealth(playerid, health);
		return 1;
	}

	if(Oyuncu[playerid][Oyunda] == true && Oyuncu[damagedid][Polis] == true && Oyuncu[playerid][Suspect] == true && SuspectAtes == false)
	{
		YollaHataMesaj(damagedid, "Ateþ açmayan þüpheliye ateþ açamazsýnýz.");
		SetPlayerHealth(playerid, health);
		return 1;
	}

	if(!IsPlayerInAnyVehicle(damagedid) && Oyuncu[playerid][Oyunda] == true && Oyuncu[damagedid][Oyunda] == true && Oyuncu[playerid][Polis] == true && Oyuncu[damagedid][Polis] == true && Oyuncu[playerid][Suspect] == false && Oyuncu[damagedid][Suspect] == false)
	{
		YollaHataMesaj(damagedid, "Takým arkadaþýna ateþ açamazsýn.");
		SetPlayerHealth(playerid, health);
		return 1;
	}
	if(Oyuncu[playerid][Oyunda] == true && Oyuncu[damagedid][Oyunda] == true && Oyuncu[playerid][Suspect] == true && Oyuncu[damagedid][Suspect] == true && Oyuncu[playerid][Polis] == false && Oyuncu[damagedid][Polis] == false)
	{
		YollaHataMesaj(damagedid, "Takým arkadaþýna ateþ açamazsýn.");
		SetPlayerHealth(playerid, health);
		return 1;
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(OyunBasladi == true && Oyuncu[playerid][Polis] == true)
	{
		...
		if(IsValidDynamic3DTextLabel(Oyuncu[playerid][YaraliTag])) DestroyDynamic3DTextLabel(Oyuncu[playerid][YaraliTag]);
		ResetPlayerWeapons(playerid);
		if(reason != 0)
		{
			if(killerid == INVALID_PLAYER_ID) YollaIpucuMesaj(playerid, "Çeþitli nedenlerden dolayý öldün.");
			...
			switch(listitem)
			{
			    case 0:
			    {
	              ...
	              if(count > 10 && !Oyuncu[playerid][Yonetici])
	                return YollaHataMesaj(playerid, "LVPD deathmatch lobisi dolu."); //sýnýr koyma
...
CMD:a(playerid, params[])
{
	...
	if(Oyuncu[playerid][SQLID] == 1 || Oyuncu[playerid][SQLID] == 33) //|| Oyuncu[playerid][SQLID] ==  || Oyuncu[playerid][SQLID] == 772)
	{
...
CMD:gotomap(playerid, params[])
{
    if(!IsPlayerInAnyVehicle(playerid)) {
        SetPlayerInterior(playerid, 0);
        SetPlayerVirtualWorld(playerid, 0);
        SetPlayerPos(playerid, Oyuncu[playerid][pClickedX], Oyuncu[playerid][pClickedY], Oyuncu[playerid][pClickedZ] + 1.5);
    }
	else
	{
        new veh = GetPlayerVehicleID(playerid);
        SetVehiclePos(veh, Oyuncu[playerid][pClickedX], Oyuncu[playerid][pClickedY], Oyuncu[playerid][pClickedZ] + 1.5);
...
CMD:dm(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_DMLOBI, DIALOG_STYLE_TABLIST_HEADERS, "Deathmatch(DM)", "Lobiler\tKiþi Sayýsý\n" "\nLas Venturas Police Department\nLiberty City Outside\nMeat Factory\nRC Battlefield", "Seç", "Kapat");
	return 1;
}
